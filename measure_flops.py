...
from rg_cnn_models import Toothless, FR_DEEP, RG_ZOO, MCRGNet

# FLOPS are measured with the TensorFlow V1 Profiler
...
def print_flops_mcrgnet(input_shape=(150, 150, 1), output_classes=3):
    # tf.compat.v1.enable_eager_execution()
    tf.compat.v1.disable_eager_execution()
    tf.compat.v1.reset_default_graph()
    ...
    with tf.compat.v1.Session(graph=tf.Graph()) as sess:
        ...
        model = MCRGNet(input_shape=input_shape, num_classes=output_classes)
        # model.compile(loss="sparse_categorical_crossentropy", optimizer="nadam", metrics=["accuracy"])
        # model.summary()
